...
export class CachedLoadConnectionsService {
  ...
  // NOTE: The order matters, so we could potentially execute the same query if in some case
  // we had one where query first then the other and in the other case it was reversed
  ...
  public processLoadAction(action: LoadAction, isFollowup: boolean = false, initiatingAction?: LoadAction, loadWaitingQueue?: string[]): CachedConnection {
    ...
    if (!keepConnected) {
      followupStream$ = connection.loadStream.pipe(
        take(1),
      );
    } else {
  ...
  private createSingleLoadConnection(action: LoadAction, initiatingAction: LoadAction): CachedConnection {
    ...
    const [entityName, collection] = this.getActionAttributes(action);
    const rawStream$ = this.db.queryListValueChanges(collection, action.queryParams, action.queryOptions).pipe(
      map((results) => {
    ...
    forkJoin([startDispatching$, rawStream$]).pipe(
      map(([_, fbPayload]) => {
        return fbPayload;
      }),
      take(1),
        ...
        return merge(
          ...
          this.db.queryListStateChanges(collection, queryParams, queryOptions).pipe(
            ...
            mergeMap((changeResults: any[]) => {
              ...
              const addedResults = changeResults.filter((r) => r.type === 'added').map((r) => r.result);

              // group remaining actions together
              const otherResults = changeResults.filter((r) => r.type !== 'added');
