...
export class CachedSelectorsService {
  ...
  private getEntityStoreSelector<T>(
    ...
    const selector = this.entityStoreSelectors[entityStoreKey] ||
  ...
  private getList<T>(
    ...
    const selectIdEqQueries = queryParams.filter((q) => q[0] === '__id' && q[1] === '==');
    const uniqSelectEqIds = selectIdEqQueries.map((q) => q[2]).filter((q, i, a) => a.indexOf(q) === i);
    const selectIdInQueries = queryParams.filter((q) => q[0] === '__id' && q[1] === 'in');
    ...
    if (uniqSelectEqIds.length === 1 && entityHash[uniqSelectEqIds[0]]) {
      ...
      filteredEntities = firstInIds.map((id) => entityHash[id]).filter((e) => e !== undefined);
