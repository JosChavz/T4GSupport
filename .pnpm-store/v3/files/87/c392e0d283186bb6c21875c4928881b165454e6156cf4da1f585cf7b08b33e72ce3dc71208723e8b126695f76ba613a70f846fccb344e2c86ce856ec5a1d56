...
import { GeneralActionTypes, Unsubscribe, ActionSetResult, ActionSet, ActionSetResponse, ActionFlow, RouterNavigate } from './app.actions';
    ...
    () => this.actions$.pipe(
      ...
      mergeMap((actionFlow: ActionFlow) => {
        ...
        const responseObsArr = actionFlow.payload.actionSets.map((actionSet) => {
          // Set a correlationId
          let correlationId;
          if (!actionSet.action['correlationId']) {
            correlationId = this.db.createId();
            actionSet.action['correlationId'] = correlationId;
          } else {
            correlationId = actionSet.action['correlationId'];
          }
          ...
          if (!actionSet.responseActionTypes) {
            ...
            return this.actions$.pipe(
              ofType(...filterTypes),
              this.eh.hasCorrelationId(correlationId),
              first(),
              map((responseAction) => {
